function get_asset_data(){
    ...
    .then((asset_data)=> {
        ...
        for (var i = 0; i< asset_data.length; i++){
            ...
            var text1 = document.createTextNode(asset_data[i]['Market']);
            var text2 = document.createTextNode(parseFloat(asset_data[i]['Value']).toFixed(3)+' +/- '+ parseFloat(asset_data[i]['Confidence']).toFixed(2));
